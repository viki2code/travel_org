...
def search_travel_plan():
    ...
    if form.validate_on_submit():
        ...
        if form.country_field.data is not None:
            travel_plans = Travel_plan.query.filter(
                (Travel_plan.country_id == form.country_field.data.id) & (
                    Travel_plan.user_id == current_user.id) & (
                    Travel_plan.date_end >= notNone(
                        form.date_start.data, '01/01/1900')) & (
                    Travel_plan.date_start <= notNone(
                        form.date_end.data, '01/01/2100'))).all()
        else:
            travel_plans = Travel_plan.query.filter(
                (Travel_plan.user_id == current_user.id) & (
                    Travel_plan.date_end >= notNone(
        ...
        if travel_plans:
            ...
            for travel_plan in travel_plans:
                ...
                plan = [
                    country, DateFormat(
                        travel_plan.date_start), DateFormat(
                        travel_plan.date_end), travel_plan.id]
